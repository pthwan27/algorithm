...
public class BJ_17472_다리만들기2_박태환 {
	static class IsLand {
		int r, c, num;
			...
			for (int mc = 0; mc < M; mc++) {
				if (map[mr][mc] == 1 && !isSelected[mr][mc]) {
					...
					while (!islandQ.isEmpty()) {
						...
						int r = pollIsland.r;
						int c = pollIsland.c;
						if (isIn(r, c) && !isSelected[r][c] && map[r][c] == 1) {
							isSelected[r][c] = true;
							map[r][c] = islandCnt;
							IsLand newIsLand = new IsLand(r, c, islandCnt);
	...
	//kruskal , 그래프 내의 모든 정점들을 가장 적은 비용으로 연결
	private static int kruskal(ArrayList<Bridge> bridgeList) {
		...
		// 섬의 갯수-1보다 union 횟수보다 적으면 모든 다리가 연결된 것이 아님.
		return unionCount == islandCnt-1 ? result : -1;
	}
